...
import barley.ndatasource.core.{CarbonDataSource, DataSourceManager}
...
class AnalyticsJDBCRelationProvider extends RelationProvider {
  ...
  override def createRelation(
    ...
    val tenantId = PrivilegedBarleyContext.getThreadLocalCarbonContext.getTenantId()
  ...
  private def resolveDataSource(tenantId: Int, dsName: String): Try[String] = {
    import barley.analytics.datasource.core.internal.{ServiceHolder => DSServiceHolder}
  ...
  def initializeTenantDS(tenantId: Int): Unit = {
    import barley.analytics.spark.core.internal.{ServiceHolder => SparkServiceHolder}
    try {
